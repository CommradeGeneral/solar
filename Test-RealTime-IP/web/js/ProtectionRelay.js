/**
 * ProtectionRelay.js — ProtectionRelay monitoring page
 * Connects via Socket.IO, filters tags by prefix PRxxx_,
 * renders analog values + digital LED statuses dynamically.
 *
 * PREFIX is page-specific (this page uses "PR").
 * Other pages can copy this pattern with a different PREFIX (e.g. "PM", "EM").
 */

(() => {
    const PREFIX = 'PR';  // ← change per page type
    const DIGITAL_COLLAPSE_STORAGE_KEY = 'PR_digital_collapsed';
    ...
    const $index   = () => document.getElementById('PR-index');
    ...
    /** Build prefix string e.g. "PR001_" */
    ...
    /** Get suffix after prefix: "PR001_ACVoltage" → "ACVoltage" */
    ...
    function updateURL() {
        const u = `ProtectionRelay.html?index=${currentIndex}${cssId ? '&css=' + cssId : ''}`;
    ...
    function setupLimits() {
        ...
        if (cssId) {
            ...
            if (css && css.ProtectionRelays) {
                minIndex = css.ProtectionRelays.startIndex || 1;
                maxIndex = css.ProtectionRelays.endIndex || (minIndex + (css.ProtectionRelays.count || 1) - 1);
            }
        } else {
            minIndex = 1;
            // total ProtectionRelays across all CSS
            let total = 0;
            (config.css || []).forEach(c => { total += c.ProtectionRelays?.count || 0; });
            maxIndex = total || 14;
        }
    ...
    function render() {
        ...
        if (analogTags.length > 0) {
            html += '<section class="PR-section analog-section">';
        ...
        if (digitalTags.length > 0) {
            html += '<section class="PR-section digital-section">';
            html += `
    ...
    function setIndex(val) {
        ...
        if (window.LoadingOverlay) {
            window.LoadingOverlay.show(2000, `Loading Protection Relay ${currentIndex}`);
        }
        ...
        subscribeToCurrentPrefix();
        render();
        notify(`Switched to ProtectionRelay ${currentIndex}`, 'info');
    }
