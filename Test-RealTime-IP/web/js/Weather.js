/**
 * WeatherStation.js — WeatherStation monitoring page
 * Connects via Socket.IO, filters tags by prefix WSxxx_,
 * renders analog values + digital LED statuses dynamically.
 *
 * PREFIX is page-specific (this page uses "WS").
 * Other pages can copy this pattern with a different PREFIX (e.g. "PM", "EM").
 */

(() => {
    const PREFIX = 'WS';  // ← change per page type
    const DIGITAL_COLLAPSE_STORAGE_KEY = 'WS_digital_collapsed';
    const USE_SOCKET_PREFIX_ROOMS = true; // subscribe to this device's prefix room (like inverter/EM/PR pages) instead of the full ALL_ROOM firehose
    ...
    function ensureScadaClientShim() {
        ...
        window.scadaClient = {
            getTagValue: (name) => {
                if (!latestValues) return undefined;
                const v = latestValues[name];
                if (typeof v === 'string') {
    ...
    /** Build prefix string e.g. "WS001_" */
    ...
    /** Get suffix after prefix: "WS001_ACVoltage" → "ACVoltage" */
    ...
    function updateURL() {
        const u = `WeatherStation.html?index=${currentIndex}${cssId ? '&css=' + cssId : ''}`;
    ...
    function setupLimits() {
        ...
        if (cssId) {
            ...
            if (css && css.WeatherStations) {
                minIndex = css.WeatherStations.startIndex || 1;
                maxIndex = css.WeatherStations.endIndex || (minIndex + (css.WeatherStations.count || 1) - 1);
            }
        } else {
            minIndex = 1;
            // total WeatherStations across all CSS
            let total = 0;
            (config.css || []).forEach(c => { total += c.WeatherStations?.count || 0; });
            maxIndex = total || 1;
        }
    ...
    function render() {
        ...
        // Keep Excel-defined order (Object property insertion order from server init)
        ...
        if (analogTags.length > 0) {
            html += '<section class="WS-section analog-section">';
        ...
        if (digitalTags.length > 0) {
            html += '<section class="WS-section digital-section">';
            html += `
    ...
    function connectSocket() {
        ...
        socket.on('disconnect', () => {
            updateConn(false);
            // On reconnect the server puts us back in ALL_ROOM and drops our prefix
            // room; clear this so 'connect' re-subscribes (otherwise the guard in
            // subscribeToCurrentPrefix would skip it and the page would get no data).
    ...
    function subscribeToCurrentPrefix() {
        if (!USE_SOCKET_PREFIX_ROOMS) return;
        if (!socket) return;
    ...
    function setIndex(val) {
        ...
        subscribeToCurrentPrefix();
        render();
        notify(`Switched to WeatherStation ${currentIndex}`, 'info');
    }
