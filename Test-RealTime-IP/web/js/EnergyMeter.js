/**
 * EnergyMeter.js — EnergyMeter monitoring page
 * Connects via Socket.IO, filters tags by prefix EMxxx_,
 * renders analog values + digital LED statuses dynamically.
 *
 * PREFIX is page-specific (this page uses "EM").
 * Other pages can copy this pattern with a different PREFIX (e.g. "PM", "EM").
 */

(() => {
    const PREFIX = 'EM';  // ← change per page type
    const DIGITAL_COLLAPSE_STORAGE_KEY = 'EM_digital_collapsed';
    ...
    const $index   = () => document.getElementById('EM-index');
    const $range   = () => document.getElementById('idx-range');
    const $label   = () => document.getElementById('EM-label');
    ...
    /** Build prefix string e.g. "EM001_" */
    ...
    /** Get suffix after prefix: "EM001_ACVoltage" → "ACVoltage" */
    ...
    function updateURL() {
        const u = `EnergyMeter.html?index=${currentIndex}${cssId ? '&css=' + cssId : ''}`;
    ...
    function setupLimits() {
        ...
        if (cssId) {
            ...
            if (css && css.EnergyMeters) {
                minIndex = css.EnergyMeters.startIndex || 1;
                maxIndex = css.EnergyMeters.endIndex || (minIndex + (css.EnergyMeters.count || 1) - 1);
            }
        } else {
            minIndex = 1;
            // total EnergyMeters across all CSS
            let total = 0;
            (config.css || []).forEach(c => { total += c.EnergyMeters?.count || 0; });
            maxIndex = total || 14;
        }
    ...
    function render() {
        ...
        if (analogTags.length > 0) {
            html += '<section class="EM-section analog-section">';
        ...
        if (digitalTags.length > 0) {
            html += '<section class="EM-section digital-section">';
            html += `
    ...
    function setIndex(val) {
        ...
        if (window.LoadingOverlay) {
            window.LoadingOverlay.show(2000, `Loading EnergyMeter ${currentIndex}`);
        }
        const inp = $index();
        ...
        subscribeToCurrentPrefix();
        render();
        notify(`Switched to EnergyMeter ${currentIndex}`, 'info');
    }
